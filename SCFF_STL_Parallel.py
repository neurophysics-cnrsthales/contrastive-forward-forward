...
def main(device_num,tr_and_eval 
         ...
         , act,concats,period,alleps,seed_num):
    """
    ...
    if save_model:
        for i, net in enumerate(nets):
            torch.save(net.state_dict(), f'./results/params_STL_l{i}.pth')
    return tacc
...
def get_arguments():
    ...
    parser.add_argument("--lr", nargs='+', type=float, default=[0.02, 0.001, 0.0004, 0.0001], help="Learning rate per layer")
    parser.add_argument("--gamma", nargs='+', type=float, default=[0.9, 0.99, 0.9, 1], help="LR decay rate per layer")
    parser.add_argument("--period", nargs='+', type=int, default=[1000, 1000, 1000, 1000], help="LR decay rate period")
    parser.add_argument("--weight_decay", nargs='+', type=float, default=[0.001, 0.001, 0, 0.0003], help="Weight decay")
    parser.add_argument("--lamda", nargs='+', type=float, default=[0, 0, 0, 0], help="Regularization lambda")
    
    # Threshold values for contrastive loss
    parser.add_argument("--th1", nargs='+', type=int, default=[0, 4, 7, 6], help="Positive sample thresholds")
    parser.add_argument("--th2", nargs='+', type=int, default=[3, 5, 8, 9], help="Negative sample thresholds")

    # Model structure and training
    parser.add_argument("--NL", type=int, default=4, help="Number of layers")
    parser.add_argument("--concats", type=tuple, default=(1, 1, 1, 1), help="Concatenation setting per layer")
    parser.add_argument("--act", nargs='+', type=str, default=["triangle", "triangle", "relu", "triangle"], help="Activation per layer")
    parser.add_argument("--alleps", nargs='+', type=int, default=[4, 8, 12, 21], help="Epochs per layer")
