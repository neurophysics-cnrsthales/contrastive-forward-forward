...
from torch.optim.lr_scheduler import ExponentialLR, StepLR, LinearLR, CosineAnnealingLR
...
def get_train(batchsize):
    ...
    valloader = torch.utils.data.DataLoader(val_data, batch_size=1000, 
                                                    shuffle=False, num_workers=0)

    sup_trainloader = torch.utils.data.DataLoader(train_data, batch_size=256, 
                                                    shuffle=True, num_workers=2)
    
    return trainloader, valloader, valloader, sup_trainloader
...
def build_classifier(lengths, config):
    classifier = nn.Sequential(
        ...
        nn.Linear(lengths, 200)  # Assuming output dimension of 10
    ).to(config.device)
        ...
        with torch.no_grad():
            for j, net in enumerate(nets):
                ...
                if j in config.Layer_out:
                    #outputs.append(out)
                    outputs.append(out.view(out.size(0), -1))
...
def accuracy(output, target, topk=(1,5)):
    ...
    target: True labels
    topk: Tuple of top-k values to calculate accuracy for
    """
    with torch.no_grad():
        maxk = max(topk)
        ...
        _, pred = output.topk(maxk, dim=1, largest=True, sorted=True)  # Get top-k indices
        ...
        return accs  # Returns a list of accuracies (top-1, top-5)
...
def test_readout(classifier, nets, pool, extra_pool, loader, criterion, config, epoch, mode):
    ...
    with torch.no_grad():
        for i, (x, labels) in enumerate(loader):
            ...
            top1_acc += acc1.item() * labels.size(0)
            top5_acc += acc5.item() * labels.size(0)
...
def create_layer(layer_config,opt_config, load_params, device, act):
    ...
    if load_params:
        net.load_state_dict(torch.load('./results/params_TIMGNET_layerwise_2_bt200_l' + str(layer_num) +'.pth', map_location='cpu'))
...
def hypersearch(dims, dims_in, dims_out, Batchnorm, epochs
    ...
    ,lr,weight_decay,gamma,threshold1,threshold2,lamda,period,out_dropout,act, concats, alleps,freezelayer):
    """
    ...
    Args:
        ...
        freezelayer (int): number of previous layers' weights to be frozon
    ...
    for i, (layer_config, opt_config) in enumerate(zip(config['IMAGENET']['layer_configs'][:NL], config['IMAGENET']['opt_configs'][:NL])):
        if i < freezelayer:
            load_params = True
        else:
            load_params = False
...
def main(device_num,tr_and_eval 
         ...
         ,act,concats,period, out_dropout,
         alleps, seed_num, freezelayer):
    """
    ...
    Args:
        ...
        period (list): Scheduler update periods per layer.
        alleps (list): Epochs per layer.
        out_dropout: Dropout rate of the final linear classifier
    ...
    if save_model:
        for i, net in enumerate(nets):
            torch.save(net.state_dict(), f'./results/params_TIMGNET_layerwise_2_bt200_best_l{i}.pth')
    return tacc
...
def get_arguments():
    ...
    parser.add_argument("--lr", nargs='+', type=float, default=[0.011, 0.001, 0.00175, 0.0002, 0.0003], help="Learning rate per layer")
    parser.add_argument("--gamma", nargs='+', type=float, default=[0.99, 0.8, 1, 0.99, 1], help="LR decay rate per layer")
    parser.add_argument("--period", nargs='+', type=int, default=[500, 500, 500, 500, 500], help="LR decay rate period")
    parser.add_argument("--weight_decay", nargs='+', type=float, default=[0, 0, 0, 0.001, 0.0001], help="Weight decay")
    parser.add_argument("--lamda", nargs='+', type=float, default=[0, 0, 0, 0, 0], help="Regularization lambda")
    
    # Threshold values for contrastive loss
    parser.add_argument("--th1", nargs='+', type=int, default=[0, 3, 5, 5, 3], help="Positive sample thresholds")
    parser.add_argument("--th2", nargs='+', type=int, default=[2, 4, 8, 9, 7], help="Negative sample thresholds")

    # Model structure and training
    parser.add_argument("--NL", type=int, default=5, help="Number of layers")
    parser.add_argument("--concats", type=tuple, default=(1, 0, 0, 1, 0), help="Concatenation setting per layer")
    parser.add_argument("--act", nargs='+', type=str, default=["triangle", "triangle", "triangle", "triangle", "relu"], help="Activation per layer")
    parser.add_argument("--alleps", nargs='+', type=int, default=[6, 15, 17, 19, 25], help="Epochs per layer")
    parser.add_argument("--out_dropout", type=float, default=0.3, help="Dropout rate of the final linear classifier")
    parser.add_argument("--freezelayer", type=int, default=2, help="number of previous layers' weights to be frozon")
...
if __name__ == "__main__":
    ...
    # Load data
    loaders = get_train(batchsize=200)
    # Run training
    tsacc = main(
